...
/*
IMPORTANT: I did some modification, now the frame_id ois not /base_link but /world
...
public:
	...
	CCalc_Grasppoints(string name) :
	    ...
	    action_name_(name)
	{
		nh_.param("base_frame",base_frame,BASE_FRAME);
		...
		this->base_tf = base_frame;//it was /base_link
...
void CCalc_Grasppoints::read_pc_cb(const haf_grasping::CalcGraspPointsServerGoalConstPtr &goal)
{
	...
	//ORIGINAL-> //bool foundTransform = tf_listener.waitForTransform(/*req.frame_id_desired.data.c_str()*/"/base_link", /*req.frame_id_original.data.c_str()*/orig_tf, now, ros::Duration(1.0));
	bool foundTransform = tf_listener.waitForTransform(/*req.frame_id_desired.data.c_str()*/base_frame, /*req.frame_id_original.data.c_str()*/orig_tf, ros::Time(0), ros::Duration(5.0));
	if (!foundTransform)
	{
	...
	//ORIGINAL -> //pcl_ros::transformPointCloud(/*req.frame_id_desired.data.c_str()*/"/base_link", pcl_cloud_in_old_cs, pc_new_cs, tf_listener);
	pcl_ros::transformPointCloud(/*req.frame_id_desired.data.c_str()*/base_frame, pcl_cloud_in_old_cs, pc_new_cs, tf_listener);
